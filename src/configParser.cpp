...
/*   Updated: 2023/10/25 19:40:04 by dodo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool configParser::validConfig(int argc, char **argv)
{
	...
	try
	{
		...
		_file.close();
		if (CGI_TIMEOUT < 1000 || CGI_TIMEOUT >= 2147483648)
			throw std::invalid_argument("invalid CGI timeout");
	}
...
void configParser::setGlobal()
{
	...
	if ((line_first_token == "timeout") && validate_directive_single(_line))
	{
		...
		else
		{
			...
			if (_settings.timeout < 1000 || _settings.timeout > 10000)
				throw std::runtime_error("Invalid Timeout value! Range(1000-10000)");
...
{
	_default_error_map.insert ( std::pair<int,std::string>(NOT_IMPLEMENTED, NOT_IMPLEMENTED_PATH));
	_default_error_map.insert ( std::pair<int,std::string>(OK, PATH_SUCCESS) );
	...
	_default_error_map.insert ( std::pair<int,std::string>(CGI_ERROR, CGI_ERROR_PATH) );
}
...
std::string configParser::handle_redirection(const std::string route, Server& server)
{
	...
	size_t start = redirected_url.find(_current_route);
	redirected_url.erase(start,_current_route.size());
	redirected_url.insert(start, return_route(server, route)->second._redirect);
